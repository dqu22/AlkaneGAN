...
def read_xyz_file(filename,tol=1e-15):
    """ This function was slighly modiffied by ttm, to handle xyz from gdb9"""
    atomic_symbols = []
    xyz_coordinates = []
    partial_charges = []
    with open(filename, "r") as file:
        for line_number,line in enumerate(file):
            if line_number == 0:
                ...
                if n <  num_atoms:
                    atomic_symbol, x, y, z, partial = line.split()
                    atomic_symbols.append(atomic_symbol)
                    xyz_coordinates.append([float(x),float(y),float(z)])
                    partial_charges.append(float(partial))
                else:
                    break
    if abs(sum(partial_charges)) < tol:
        charged_fragments = True
    else:
        charged_fragments = False
    atomicNumList = get_atomicNumList(atomic_symbols)
    return atomicNumList,charge,xyz_coordinates,charged_fragments
...
def chiral_stereo_check(mol):
    ...
    Chem.rdmolops.DetectBondStereochemistry(mol,-1)
